#!/bin/bash
# scripts/rollback.sh
# DEV-ONLY rollback script. Mirrors the logic used in
...
#
# Requires: RAILWAY_TOKEN_DEV (a Railway *project* access token for the
# dev environment) to be set in your shell before running, e.g.:
...
ENVIRONMENT="dev"
SERVICE="django555"
HEALTH_URL="https://django555-dev.up.railway.app/health/"
REASON="${1:-Manual rollback}"

if [ -z "${RAILWAY_TOKEN_DEV:-}" ]; then
  echo "❌ RAILWAY_TOKEN_DEV is not set. Export it first:"
  echo '   export RAILWAY_TOKEN_DEV="your-dev-project-token"'
  exit 1
fi

echo "═══════════════════════════════════════════════"
echo "  ROLLBACK SCRIPT (dev only)"
...
RAILWAY_TOKEN="$RAILWAY_TOKEN_DEV" railway deployment list \
  --service "$SERVICE" \
...
RESPONSE=$(curl -s -X POST https://backboard.railway.com/graphql/v2 \
  -H "Project-Access-Token: $RAILWAY_TOKEN_DEV" \
