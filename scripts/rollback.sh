#!/bin/bash
# scripts/rollback.sh
# PRODUCTION-ONLY rollback script. Mirrors the logic used in
...
#
# Requires: RAILWAY_TOKEN_PRODUCTION (a Railway *project* access token for the
# production environment) to be set in your shell before running, e.g.:
#   export RAILWAY_TOKEN_PRODUCTION="xxxxx"

set -euo pipefail

ENVIRONMENT="production"
SERVICE="gregarious-purpose"
HEALTH_URL="https://gregarious-purpose-production-98d4.up.railway.app/health/"
REASON="${1:-Manual rollback}"

if [ -z "${RAILWAY_TOKEN_PRODUCTION:-}" ]; then
  echo "❌ RAILWAY_TOKEN_PRODUCTION is not set. Export it first:"
  echo '   export RAILWAY_TOKEN_PRODUCTION="your-production-project-token"'
  exit 1
fi

echo "═══════════════════════════════════════════════"
echo "  ROLLBACK SCRIPT (production only)"
...
RAILWAY_TOKEN="$RAILWAY_TOKEN_PRODUCTION" railway deployment list \
  --service "$SERVICE" \
